...
if [ -n "${BRIGHTNESS_PERCENT}" ];then
 find /sys/class/backlight/ -type l -exec bash -c "echo \\\$(( \\\$(cat {}/max_brightness) * ${BRIGHTNESS_PERCENT} / 100 )) | sudo tee {}/brightness" \;
else
 find /sys/class/backlight/ -type l -exec bash -c "echo \\\$(( \\\$(cat {}/max_brightness) * 100 / 100 )) | sudo tee {}/brightness" \;
fi
EOF
