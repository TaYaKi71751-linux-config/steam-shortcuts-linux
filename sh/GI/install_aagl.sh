...
function system_install_latest_flatpak_package(){
	PACKAGE_NAME="$1"
RESULT="$(bash << EOF
flatpak install --system ${PACKAGE_NAME} --assumeyes
EOF
...
if ( echo ${RESULT} | grep already\ installed );then
	...
	LATEST_LIST="$(echo "${RESULT}" | grep "[" | grep "]" | cut -d '-' f1 | rev | cut -d ']' | rev)"
	PACKAGE_LINES="$(echo "${RESULT}" | grep "${PACKAGE_NAME}/" | grep "${LATEST_LIST})")"
while IFS= read -r line
do
bash << EOF
	flatpak install --system $(echo $line | rev | cut -d ' ' -f1 | rev) --assumeyes
...
function user_install_latest_flatpak_package(){
	PACKAGE_NAME="$1"
RESULT="$(bash << EOF
flatpak install --user ${PACKAGE_NAME} --assumeyes
EOF
...
system_install_latest_flatpak_package org.gnome.Platform//49
user_install_latest_flatpak_package moe.launcher.an-anime-game-launcher
