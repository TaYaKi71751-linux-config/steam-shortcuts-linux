...
const __GAME_NAME__ = 'Proton';
const __OUT_NAME__ = 'proton';
const outPath = path.join(`${process.env.PWD}`, 'out', __OUT_NAME__);
let apps = [
	{AppName: 'Install dwproton-bin', exe: path.join(outPath, 'install_dwproton.out'), StartDir: outPath, LaunchOptions: '%command%', tags: [__GAME_NAME__]},
];
	// Lutris
	{
